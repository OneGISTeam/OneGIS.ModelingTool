...
public:
	...
	*/
	...
	virtual void setMessageCallBack( void* obj_ptr, void (*func)(void*, shared_ptr<Message> t) )
	{
		...
		// pass to children
        for( std::vector<StatusCallback*>::iterator it = m_callback_children.begin(); it != m_callback_children.end(); ++it )
		{
	...
	{
        for( std::vector<StatusCallback*>::iterator it = m_callback_children.begin(); it != m_callback_children.end(); )
		{
			StatusCallback* child = *it;
