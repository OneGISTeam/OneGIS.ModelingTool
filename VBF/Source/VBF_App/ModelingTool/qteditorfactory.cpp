...
class QtPopupEditorWidget : public QWidget {
    Q_OBJECT

public:
    QtPopupEditorWidget(QWidget *parent);
...
QtPopupEditorWidget::QtPopupEditorWidget(QWidget *parent) :
    QWidget(parent),
    m_button(new QToolButton),
	m_valuetext( new QLineEdit )
{
	m_valuetext->setReadOnly( false );
	m_valuetext->selectAll();
	
	QHBoxLayout *lt = new QHBoxLayout(this);
	setupTreeViewEditorMargin(lt);
	lt->setSpacing(0);	
	lt->addWidget(m_valuetext);
	lt->addItem(new QSpacerItem(0, 0, QSizePolicy::Expanding, QSizePolicy::Ignored));
	...
	m_button->installEventFilter(this);
	connect(m_button, SIGNAL(clicked()), this, SLOT(buttonClicked()));
	//connect(m_valuetext, SIGNAL(editingFinished()),this, SLOT(slotEditingFinished()));

	lt->addWidget(m_button);
}

// The line editor has finished text editing, notify its parent
void QtPopupEditorWidget::slotEditingFinished()
{
	emit valueChanged(m_valuetext->text() );
}


void QtPopupEditorWidget::setValue(const QString &f)
{
	m_valuetext->setText( f );
}

void QtPopupEditorWidget::buttonClicked()
{
// 	QMessageBox::information(NULL, "Title", "Content", QMessageBox::Yes | QMessageBox::No, QMessageBox::Yes);
//   
 //	setValue(QString(tr("changed")));
	//m_valuetext->setText("");
	QString text = m_valuetext->text();
	emit valueChanged(m_valuetext->text() );
    
}

bool QtPopupEditorWidget::eventFilter(QObject *obj, QEvent *ev)
{
    if (obj == m_button) {
        switch (ev->type()) {
        case QEvent::KeyPress:
...
// QtPopupEditorFactoryPrivate

class QtPopupEditorFactoryPrivate : public EditorFactoryPrivate<QtPopupEditorWidget>
{
    QtPopupEditorFactory *q_ptr;
    Q_DECLARE_PUBLIC(QtPopupEditorFactory)
public:
...
void QtPopupEditorFactoryPrivate::slotPropertyChanged(QtProperty *property,
                const QString &value)
{
    const PropertyToEditorListMap::iterator it = m_createdEditors.find(property);
    if (it == m_createdEditors.end())
        return;
    QListIterator<QtPopupEditorWidget *> itEditor(it.value());

    while (itEditor.hasNext())
        itEditor.next()->setValue(value);
}

void QtPopupEditorFactoryPrivate::slotSetValue(const QString &value)
{
    QObject *object = q_ptr->sender();
    const EditorToPropertyMap::ConstIterator ecend = m_editorToProperty.constEnd();
    for (EditorToPropertyMap::ConstIterator itEditor = m_editorToProperty.constBegin(); itEditor != ecend; ++itEditor)
        if (itEditor.key() == object) {
            ...
            if (!manager)
                return;
           // manager->setValue(property, value); //陈达更改，用下一句替代改句
			manager->emitEditDialogSignal(property,value);
            return;
        }
}

/*!
    \class QtPopupEditorFactory

    \brief The QtPopupEditorFactory class provides font editing for
    properties created by QtFontPropertyManager objects.
...
/*!
    Creates a factory with the given \a parent.
*/
QtPopupEditorFactory::QtPopupEditorFactory(QObject *parent) :
QtAbstractEditorFactory<QtPathPropertyManager>(parent),
    d_ptr(new QtPopupEditorFactoryPrivate())
{
    d_ptr->q_ptr = this;
}
...
/*!
    Destroys this factory, and all the widgets it has created.
*/
QtPopupEditorFactory::~QtPopupEditorFactory()
{
    qDeleteAll(d_ptr->m_editorToProperty.keys());
    delete d_ptr;
}
...
/*!
    ...
    Reimplemented from the QtAbstractEditorFactory class.
*/
void QtPopupEditorFactory::connectPropertyManager(QtPathPropertyManager *manager)
{
    connect(manager, SIGNAL(valueChanged(QtProperty*,const QString &)),
            this, SLOT(slotPropertyChanged(QtProperty*,const QString &)));
}
...
/*!
    ...
    Reimplemented from the QtAbstractEditorFactory class.
*/
QWidget *QtPopupEditorFactory::createEditor(QtPathPropertyManager *manager,
        QtProperty *property, QWidget *parent)
{
    QtPopupEditorWidget *editor = d_ptr->createEditor(property, parent);
    editor->setValue(manager->value(property));
    connect(editor, SIGNAL(valueChanged(const QString &)), this, SLOT(slotSetValue(const QString &)));
    connect(editor, SIGNAL(destroyed(QObject *)), this, SLOT(slotEditorDestroyed(QObject *)));
    return editor;
}
...
/*!
    ...
    Reimplemented from the QtAbstractEditorFactory class.
*/
void QtPopupEditorFactory::disconnectPropertyManager(QtPathPropertyManager *manager)
{
