...
class QT_QTPROPERTYBROWSER_EXPORT QtPathPropertyManager : public QtAbstractPropertyManager
{
	Q_OBJECT
public:
	...
	QString value(const QtProperty *property) const;
	void emitEditDialogSignal(QtProperty* property,const QString& val){emit showEditDialogSignal(property,val);}
	public Q_SLOTS:
		void setValue(QtProperty *property, const QString &val);
		
Q_SIGNALS:
		void valueChanged(QtProperty *property, const QString &val);
		void showEditDialogSignal(QtProperty*,const QString&);
...
protected:
	...
	virtual void uninitializeProperty(QtProperty *property);
private:
	QtPathPropertyManagerPrivate *d_ptr;
	Q_DECLARE_PRIVATE(QtPathPropertyManager)
		Q_DISABLE_COPY(QtPathPropertyManager)
};
