...
public:
    int _timerId;
    CVBF_Timer _lastFrameStartTime;
    observer_ptr<IVBF_Viewer> _viewer;

    HeartBeat();
    virtual ~HeartBeat();
    void init( IVBF_Viewer *viewer );
    void stopTimer();
...
public:
    ...
    virtual osg::GraphicsContext* createGraphicsContext( osg::GraphicsContext::Traits* traits )
    {
        ...
        {
            ref_ptr< GraphicsWindowQt > window = new GraphicsWindowQt( traits );
...
void setViewer( IVBF_Viewer *viewer )
{
...
void HeartBeat::init( IVBF_Viewer *viewer )
{
    if( _viewer == viewer )
        return;
...
void HeartBeat::timerEvent( QTimerEvent *event )
{
    ...
    else
    {
        ...
        if( viewer->getRunFrameScheme() == CVBF_Viewer::ON_DEMAND )
        {
            double dt = _lastFrameStartTime.time_s();
            if (dt < 0.01)
