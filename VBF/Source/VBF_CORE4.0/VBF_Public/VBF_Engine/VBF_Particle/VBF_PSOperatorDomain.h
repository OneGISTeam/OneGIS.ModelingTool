...
// 区域操作器，接受不同的区域形状作为参数可以用来派生实现粒子与区域相交的操作。参见David McAllister的粒子系统API (http://www.particlesystems.org)
...
public:
    ...
    // 直接添加一个区域对象。被.osg wrappers和serializers调用
    void addDomain( const Domain& domain ) { _domains.push_back(domain); }
    
    // 直接获取一个区域对象。被.osg wrappers和serializers调用
