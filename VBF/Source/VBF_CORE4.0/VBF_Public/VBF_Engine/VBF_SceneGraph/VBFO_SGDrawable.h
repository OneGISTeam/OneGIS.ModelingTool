...
// USE_SEPARATE_COMPILE_AND_EXECUTE 用于改变Draw函数中显示列表编译的方式。
// 据发现，NVidia drivers 优化COMPILE_AND_EXECUTE完全失败，比没用显示列表还慢。
// 但是把COMPILE和EXECUTE分开，则优化效果良好?!  
// 定义为默认方式，先COMPILE再glCallList，而不是使用COMPILE_AND_EXECUTE。

#define USE_SEPARATE_COMPILE_AND_EXECUTE



/** 可绘制对象的基类。Drawable类中没有drawing primitives，在其子类中提供。
 *  注意：Drawable不是节点，不能直接加入到场景图中。需要把它先加入到一个叶节点中，再加入到场景图。
 *  Drawable有一个StateSet的智能指针，StateSet可以被不同的Drawable共享。 
 *  共享StateSet有利于提高性能，可以减少OpenGL state切换的次数和开销。
 *  Drawable也可以被不同的叶节点共享，因此，相同的geometry (仅装入内存一次) 可以被用于场景图的不同部分。
*/
...
*/
        ...
        // 设置/获取保留在（已删除显示列表缓存）中的显示列表的最小数量,为了重用
...
//			OSG_NOTICE<<"创建显示列表"<<globj<<std::endl;
