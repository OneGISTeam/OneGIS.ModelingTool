...
class Constraint;
class Dragger;  // 杜莹添加：2013-09-16

// 设计模式中的命令的概念。一个命令有三个主要的执行单元：接收者（Receiver），执行命令改变自身的状态；
//  	触发者（Invoker），发出命令并传给命令参数；命令本身（Command）。
...
class OSGMANIPULATOR_EXPORT MotionCommand : public ::CVBF_Referenced
{
    public:
		...
		// 设置/获取发送动作消息的Dragger，即当前正在拖动的Dragger（杜莹添加：2013-09-11）
    ...
    private:
		...
		// 记录发送动作消息的Dragger，即当前正在拖动的Dragger（杜莹添加：2013-09-11）
		// 注意：此处使用observer_ptr会导致相互包含，编译不通过
